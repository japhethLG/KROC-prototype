...
Rows marked **(6/29)** in Notes were introduced or modified by the **2026‑06‑29 prototype sync** (this round); rows marked **(5/28)** trace to the 2026‑05‑28 meeting; all other non‑🔒 rows trace to the prior (week‑of‑2026‑05‑12) review. Fields removed during design are **omitted** (see [Removed fields](#removed-fields)).
...
| Hours of Operation | Repeater (Day, Ranges[] **or** Closed) | No | 🔄 | **(6/29)** Structured: multiple time ranges per day **or** a Closed flag; whole group **optional** (no‑hours variant for rentals) (SCHEMA‑10) |
...
| Form Fields (rep) | Repeater (Field Type, Label, Placeholder, Required) | Yes | 🆕 | `text` · `email` · `select` · `textarea` |
...
| Layout | Dropdown (`4‑up cards` / `3‑up with bio` / `1‑up feature`) | No | 🆕 | |
...
| Included Features (rep) | Repeater (Text) | No | 🆕 | Comparison rows / bullet list |
...
|---|---|---|
| Page Name + Hero | `[informational_pages]` | Hero H1 + Hero Image Set (campaign hero) + Primary/Secondary CTA |
| Benefits Band | `[intro_band]` / `[donation_block]`‑style | Mission/benefits band (§19) |
| Discounts & Holds | `[faqs]` (accordion) | Reuses accordion model (§6) |
| Membership Tiers | `[rate_cards]` (`Cards`) | Tier name, Member/Public PricePoints, age range, period (§21) |
| Family Play Comparison | `[rate_cards]` (`Comparison Table`) | Comparison‑table mode (§21) |
| PlayCare Band | `[facility_section]` variant | Rates band (§11 / §21) |
| Member Story | `[featured_stories]` | Quote/story (§8) |
| Hold / Cancel Form | `[custom_forms]` | (§14) |
| Policies Band | Drag‑in band · cross‑links | Cross‑links to other `[informational_pages]` |
...
| Intro Band | Page Content (WYSIWYG) + CTA links | Intro + access notes |
...
| Section | Source | Notes |
|---|---|---|
| Page Name + Hero | `[informational_pages]` | Theatre campaign hero |
...
| Interest Form | `[custom_forms]` | OQ‑4: inquiry vs. booking system (§14) |
...
| Leadership / Team | `[people_block]` | (§15) |
| Connect Links | Page Content · colored link buttons | |
| Ministries & Schedule | `[faqs]` + schedule list | Weekly schedule; prayer‑request form = `[custom_forms]` (OQ‑5 scope) |
...
| CTA Band | `[donation_block]`‑style band | Or `[featured_volunteer_opportunities]` retitled "Join the Team" |
| Questions / HR | `[people_block]` | HR contacts (§15) |
